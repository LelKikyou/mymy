var sdcm = function ($, Sdcm) {
    ...
    var page = {
        ...
        //事件
        event: function () {
            $(".flowchart").mCustomScrollbar({ // 优化滚动条
                axis: "x",
                theme: "minimal-dark"
            });
        },
        ...
        initLoad: function () {
            ...
            var menusData = {};
            var linesData = {};
            $.each(flowchartData, function (index, item) {
                var id = item.id;
                //找出最小的pos，初始展示的页面
                if (!oldPos) {
                    ...
                    var lock = page.minPos(oldPos, item.position);
                    if (lock) {
                        oldPos = item.position;
                        minId = id;
                    }
                ...
                var direction = JSON.parse(item.direction);
                if (direction.length === 0) {
                    return true;
                }
                $.each(direction, function (inx, con) {
                    var line = con.split('-');
                    var lineQ = line[0];
                    var lineH = line[1];
                    var uuid = lineQ + lineH;
                    linesData[uuid] = {
                        flowbarmenufk: lineQ,
                        id: uuid,
                        nextflowbarmenufk: lineH,
                        type: "sl" //直线
                    }
                });
            });
    ...
    function Flowchart() {
        ...
        this._bindEvent = {
            ...
            clickEvent: function () {
                ...
                var url = data["url"];
                if (!!url) {
                    var $iframe = $('<iframe class="flowbar-iframe" id="flowbar-iframe" frameborder="0" src="' + url + '?readonly='+data.readonly+'"></iframe>');
                    var $iframeBody = $("#iframe-body");
                    $iframeBody.html($iframe);
                } else {
                    alert("url还没有配置")
                }
            }
        ...
        this._createMenus = function (menuData) {
            for (var id in menuData) {
                ...
                $menu.data("data", menu);
                $menu.click(this._bindEvent.clickEvent);
        ...
        this._addLine = function (id, json) {
            ...
            var n1 = $("#sdcm" + json.flowbarmenufk).data("data"),
                n2 = $("#sdcm" + json.nextflowbarmenufk).data("data");
            if (!n1 || !n2) return;
            var res;
