        ...
        load: function () {
            ...
            for (var i = 0, l = modules.length; i < l; i++) {
                ...
                if (!(item.children.length === 0)) {
                    menuItemHtml += '<img src="images/index/jt.png" class="sdcm-menu-item-arrow"/>';
                }
                menuItemHtml += '</a>';
                ...
                for (var j = 0, sl = secondModules.length; j < sl; j++) {
                    ...
                    if (!(secondItem.children.length === 0)) {
                        secondItemHtml += '<img src="images/index/jt.png" class="sdcm-menu-item-arrow"/>'
                    }
                    secondItemHtml += '</a>';
                    ...
                    for (var m = 0, tl = threeModules.length; m < tl; m++) {
                        ...
                        var threeItemHtml = '<a id="' + threeItem.id + '" href="javascript:void(0);" F_isState="iframe" url="' + threeItem.url + '?id='+ threeItem.id +'" class="sdcm-menu-item" >';
        ...
        bind: function () {
            ...
            $('#sdcm_frame_menu a').on('click', function () {
                ...
                switch (isState) {
                    ...
                    case 'iframe':// 窗口
                        // if(!$(this).attr("url")){
                        var iddx = $(this).attr("url").lastIndexOf("?");
                        var ralUrl = $(this).attr("url").substring(0, $(this).attr("url").length);
                        if(!ralUrl || iddx === 0){
                            alert("当前菜单没有url地址,请先配置")
                            return;
                        }
                        var _module = {
                    ...
                    case 'expand':// 打开子菜单
                        ...
                        if ($ul.is(':visible')) {
                            ...
                            if ($ul.hasClass('sdcm-second-menu-list')) {
                                $('#sdcm_frame_menu .sdcm-second-menu-list').slideUp(100, function () {
                                    $(this).prev().removeClass('open');
                                });
                            }
                            else {
                                $ul.parents('.sdcm-second-menu-list').find('.sdcm-three-menu-list').slideUp(100, function () {
                                    $(this).prev().removeClass('open');
                                });
                            }
