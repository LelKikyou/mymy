(function ($, Sdcm) {
    ...
    $.lrselect = {
        ...
        treeRender: function (dfop) {
            ...
            //用哪个数据存储现在text是name
            data[dfop.text]='==' + dfop.placeholder + '==';
            dfop.data.unshift(data)
            var treeop = {
                data: dfop.data,
        ...
        click: function (e) {
            ...
            if ($option.is(":hidden")) {
                ...
                var width = dfop.width || $this.width();
        ...
        treeNodeClick: function (item, $item) {
            ...
            $inputText.html(dfop.currtentItem[dfop.text]);
            if (item.value == '') {
                $inputText.css('color', '#999');
            }
            else {
    ...
    $.fn.lrselect = function (op) {
        ...
        // url优先级最高
        if (!!dfop.url) {
            Sdcm.httpAsync(dfop.method, dfop.url, dfop.param, function (res) {
