...
(function ($) {
    ...
    $.extend($.jfGrid, {
        ...
        renderHead: function ($self) {// 渲染表格头部
            ...
            //自动获取列表单列宽度
            var totalCount = 0;            //hcy
            for (var m = 0; m < len; m++) {
                var cellhcy = dfop.headData[m];
                if (!cellhcy.display) {
                    if (totalCount > 11) {
                        totalCount = 11;
                    } else {
            ...
            var totalWidth = $self.width() - 30;
            var curColWidth = Math.round(totalWidth / totalCount);
            if (curColWidth < 150) {
                curColWidth = 150
            }
            var flag = false;             //hcy
        ...
        renderPageBarinit: function ($self) {
            ...
            if (dfop.isPage && dfop.url) {
                ...
                var $pagebar = $('<div class="jfgrid-page-bar" id="jfgrid_page_bar_' + dfop.id + '"><div class="jfgrid-page-bar-info" >无显示数据</div><div class="jfgrid-page-hcy-wrap"></div></div>');
                $self.append($pagebar);
        ...
        renderPageBar: function ($self) {
            ...
            if (dfop.rowdatas.length == 0) {
                ...
                pagehcyWrap += '<span class="jfgrid-page-hcy-up">上一页</span><ul class="pagination pagination-sm" id="jfgrid_page_bar_num_' + dfop.id + '" >';
                pagehcyWrap += btnlist;
                pagehcyWrap += '</ul><span class="jfgrid-page-hcy-down">下一页</span><span class="jfgrid-page-hcy-last">尾页</span>';
                pagehcyWrap += '到第几<input class="jfgrid-page-hcy-input" type="text">页/' + pageparam.total;
                $pagebar.find('.jfgrid-page-hcy-wrap').html(pagehcyWrap);
                var $pagebarBtn = $pagebar.find('#jfgrid_page_bar_num_' + dfop.id);
                if (pageparam.page == 1) {
                    $pagebar.find(".jfgrid-page-hcy-up").attr("disabled", true);
                    $pagebar.find(".jfgrid-page-hcy-up").addClass("dis-color");
                }
                if (pageparam.page == pageparam.total) {
                    $pagebar.find(".jfgrid-page-hcy-down").attr("disabled", true);
                    $pagebar.find(".jfgrid-page-hcy-down").addClass("dis-color");
                }
                $pagebar.find(".jfgrid-page-hcy-input").val(pageparam.page);
                $pagebar.find(".jfgrid-page-hcy-up").on('click', $.jfGrid.reloadPageUp);
                $pagebar.find(".jfgrid-page-hcy-down").on('click', $.jfGrid.reloadPageDown);
                $pagebar.find(".jfgrid-page-hcy-last").on('click', $.jfGrid.reloadPageLast);
                $pagebar.find(".jfgrid-page-hcy-input").on('blur', $.jfGrid.reloadPageBlur);
                $pagebar.find(".jfgrid-page-hcy-input").on("keypress", function (e) {
                    if (event.keyCode == "13") {
                        var $this = $(this);
                        var $self = $('#girdtable');
                        var dfop = $self[0].dfop;
                        if (0 < dfop.pageparam.page <= dfop.pageparam.total) {
                            var page = $this.val();
                            $this.parent().find(".jfgrid_page_bar_num_girdtable").find('.active').removeClass('active');
                            $this.parent().find(".jfgrid_page_bar_num_girdtable").find(".pagebtn").eq(page - 1).addClass('active');
        ...
        reloadPageUp: function (e) {
            var $this = $(this).parent().find(".pagination ");
            var $self = $('#' + $this.attr('id').replace('jfgrid_page_bar_num_', ''));
            var dfop = $self[0].dfop;

            if (dfop.pageparam.page > 1) {
                var page = dfop.pageparam.page - 1;
                $this.find('.active').find('.active').removeClass('active');
                $this.find('.active').find(".pagebtn").eq(page - 1).addClass('active');
                dfop.pageparam.page = page;
                dfop.param['pagination'] = JSON.stringify(dfop.pageparam);
                $.jfGrid.ajaxLoad($self, dfop.url, dfop.param, function (res) {
                    if (res.code == 200) {
        ...
        reloadPageDown: function (e) {
            var $this = $(this).parent().find(".pagination ");
            var $self = $('#' + $this.attr('id').replace('jfgrid_page_bar_num_', ''));
            var dfop = $self[0].dfop;
            if (dfop.pageparam.page < dfop.pageparam.total) {
                var page = dfop.pageparam.page + 1;
                $this.find('.active').find('.active').removeClass('active');
                $this.find('.active').find(".pagebtn").eq(page - 1).addClass('active');
                dfop.pageparam.page = page;
        ...
        reloadPageLast: function (e) {
            var $this = $(this).parent().find(".pagination ");
            var $self = $('#' + $this.attr('id').replace('jfgrid_page_bar_num_', ''));
            var dfop = $self[0].dfop;
            var page = dfop.pageparam.total;
            $this.find('.active').find('.active').removeClass('active');
            $this.find('.active').find(".pagebtn").eq(page - 1).addClass('active');
            dfop.pageparam.page = page;
            dfop.param['pagination'] = JSON.stringify(dfop.pageparam);
        ...
        reloadPageBlur: function (e) {
            ...
            if (0 < dfop.pageparam.page <= dfop.pageparam.total) {
                var page = $(this).val();
                $this.find('.active').removeClass('active');
                $this.find(".pagebtn").eq(page - 1).addClass('active');
                dfop.pageparam.page = page;
                dfop.param['pagination'] = JSON.stringify(dfop.pageparam);
                $.jfGrid.ajaxLoad($self, dfop.url, dfop.param, function (res) {
                    if (res.code == 200) {
        ...
        renderRowData: function ($self, data) {
            ...
            for (var i = 0; i < len; i++) {//行循环
                ...
                for (var n = 0, nl = dfop._colModel.length; n < nl; n++) {// 右侧滚动显示区域
                    if (!!dfop._colModel[n].display) {
                        var $dcell = $('<div class="jfgrid-data-cell" rownum="' + rownum + '" colname="' + dfop._colModel[n].name + '" style="width:' + dfop._colModel[n].width + 'px;text-align:' + dfop._colModel[n].align + ';display:' + dfop._colModel[n].display + '" title="' + (_row[dfop._colModel[n].name] || "") + '"  >' + (_row[dfop._colModel[n].name] || "") + '</div>');
                    } else {
                        var $dcell = $('<div class="jfgrid-data-cell" rownum="' + rownum + '" colname="' + dfop._colModel[n].name + '" style="width:' + dfop._colModel[n].width + 'px;text-align:' + dfop._colModel[n].align + ';" title="' + (_row[dfop._colModel[n].name] || "") + '"  >' + (_row[dfop._colModel[n].name] || "") + '</div>');
                    }
        ...
        renderTreeRowData: function ($self, data) {
            ...
            function _fn(_data, deep, path) {
                for (var j = 0, l = _data.length; j < l; j++) {
                    ...
                    for (var n = 0, nl = dfop._colModel.length; n < nl; n++) {// 右侧滚动显示区域
                        if (!!dfop._colModel[n].display) {
                            var $dcell = $('<div class="jfgrid-data-cell" rownum="' + rownumindex + '" colname="' + dfop._colModel[n].name + '" style="width:' + dfop._colModel[n].width + 'px;text-align:' + dfop._colModel[n].align + ';display:' + dfop._colModel[n].display + '" title="' + (_row[dfop._colModel[n].name] || "") + '"  >' + (_row[dfop._colModel[n].name] || "") + '</div>');
                        } else {
    ...
    //判断有没有子集，删除时候用
    $.fn.jfGridRowChildrenLength =function(){
        var $self = $(this);
        var id = $self.attr('id')
        if (id == null || id == undefined || id == '') {
            return null;
        ...
        var length;
        if(!_rowdata.length){
            length=_rowdata.children.length;
            return length>0?{name:_rowdata.name,is:true}:{name:_rowdata.name,is:false};
        }else {
            for (var i=0;i<_rowdata.length;i++){
                length=_rowdata[i].children.length;
                if(length>0){
                    return {name:_rowdata[i].name,is:true}
                }
            }
            return {is:false}
        }
