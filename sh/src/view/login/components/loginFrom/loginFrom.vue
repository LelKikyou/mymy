<template>
    <div class="loginFrom">
        <div style="position: relative">
            ...
            <Input v-model="login.username" @keyup.enter.native="loginBtn" placeholder="账户" class="v-input" />
        </div>
        <div style="position: relative;margin-top: 20px;">
            <i class="iconfont icon-mima"></i>
            <Input v-model="login.password" @keyup.enter.native="loginBtn" type="password" placeholder="密码" class="v-input" />
        </div>
        ...
        methods: {
            loginBtn() {
                ...
                loginApi(this.login).then((res) => {
                    if (res.code === 200) {
                        //设置token
                        const loginInfo = this.login
                        localStorage.setItem('newShangHaiProject', JSON.stringify(loginInfo));
